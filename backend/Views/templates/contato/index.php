<div class="page-wrapper">
    ...
    <div class="page-header">
        <div class="page-header-content">
            <div class="page-title-group">
                ...
                <p class="page-subtitle">Gerencie todos os contatos recebidos</p>
            </div>
            <a href="/backend/contato/criar" class="btn-action-primary">
    ...
    <!-- Filtros -->
    <form method="GET" action="/backend/contato" id="filterForm">
        ...
        <div class="filters-section">
            <div class="filters-group">
                <div class="search-box">
                    ...
                    <input type="text" name="busca" placeholder="Buscar por nome, email..." class="search-input"
                           value="<?= htmlspecialchars($_GET['busca'] ?? '') ?>" onkeyup="autoSubmitFilter()">
                </div>
                ...
                <select class="filter-select" name="status" onchange="document.getElementById('filterForm').submit()">
                    ...
                    <option value="respondido" <?= ($_GET['status'] ?? '') === 'respondido' ? 'selected' : '' ?>>Respondido</option>
                ...
                <?php if (!empty($_GET['busca']) || !empty($_GET['status']) || !empty($_GET['periodo'])): ?>
                    <a href="/backend/contato" class="btn-filter-reset">
    ...
    <div class="stat-card stat-novo">
        ...
        <div class="stat-info">
            ...
            <span class="stat-value"><?= $stats['novo'] ?? 0 ?></span>
        </div>
    </div>

    <!-- RESPONDIDOS -->
    <div class="stat-card stat-respondido">
        <div class="stat-icon">
            <i class="bi bi-check-circle"></i>
        </div>
        <div class="stat-info">
            <span class="stat-label">Respondidos</span>
            <span class="stat-value"><?= $stats['respondido'] ?? 0 ?></span>
        </div>
    </div>
...
<?php endif; ?>
    ...
    <?php if (!empty($contatos)): ?>
        <table class="data-table" id="contatosTable">
            <thead>
                <tr>
                    <th><input type="checkbox" class="table-checkbox" id="selectAll" onchange="toggleSelectAll(this)"></th>
                    <th><div class="th-content" onclick="sortTable('id_contato')">ID <i class="bi bi-chevron-expand sort-icon"></i></div></th>
                    <th><div class="th-content" onclick="sortTable('nome_contato')">Nome <i class="bi bi-chevron-expand sort-icon"></i></div></th>
                    <th><div class="th-content" onclick="sortTable('email_contato')">Email <i class="bi bi-chevron-expand sort-icon"></i></div></th>
                    <th><div class="th-content" onclick="sortTable('assunto_contato')">Assunto <i class="bi bi-chevron-expand sort-icon"></i></div></th>
                    <th><div class="th-content" onclick="sortTable('status_contato')">Status <i class="bi bi-chevron-expand sort-icon"></i></div></th>
                    <th><div class="th-content" onclick="sortTable('data_envio')">Data <i class="bi bi-chevron-expand sort-icon"></i></div></th>
            ...
            <tbody>
                <?php foreach ($contatos as $c): ?>
                    <?php
                        $status = strtolower($c['status_contato'] ?? '');
                        $statusConfig = match ($status) {
                            'respondido' => ['class' => 'status-realizada', 'icon' => 'check-circle-fill', 'text' => 'Respondido'],
                            'novo'       => ['class' => 'status-pendente', 'icon' => 'envelope', 'text' => 'Novo'],
                    ...
                    ?>
                    <tr class="table-row">
                        <td><input type="checkbox" class="table-checkbox row-checkbox" value="<?= $c['id_contato'] ?>" onchange="updateSelection()"></td>
                        <td><span class="table-id">#<?= $c['id_contato'] ?></span></td>
                        <td>
                            <div class="client-info">
                                <div class="client-avatar"><?= strtoupper(substr($c['nome_contato'], 0, 1)) ?></div>
                                <div class="client-details">
                                    <span class="client-name"><?= htmlspecialchars($c['nome_contato']) ?></span>
                                    <span class="client-email"><?= htmlspecialchars($c['email_contato']) ?></span>
                                </div>
                            </div>
                        </td>
                        <td><?= htmlspecialchars($c['email_contato']) ?></td>
                        <td><?= htmlspecialchars($c['assunto_contato']) ?></td>
                        <td>
                        ...
                        <td class="text-end">
                            <div class="action-buttons">
                                <a href="/backend/contato/editar/<?= $c['id_contato'] ?>" class="btn-action btn-action-edit" title="Editar">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="/backend/contato/converter/<?= $c['id_contato'] ?>" method="POST" style="display:inline;">
                                    <button type="submit" class="btn-action btn-action-convert" title="Converter em Cliente"
                                            onclick="return confirm('Converter em cliente?')">
                                        <i class="bi bi-person-plus"></i>
                                    </button>
                                </form>
                                <button onclick="irParaExclusao(<?= $c['id_contato'] ?>)" 
                                class="btn-action btn-action-delete" title="Excluir">
        ...
        <div class="table-footer">
            <div class="table-info">
                Mostrando <strong><?= $paginacao['inicio'] ?? 1 ?></strong> a 
                <strong><?= $paginacao['fim'] ?? count($contatos) ?></strong> de 
                <strong><?= $paginacao['total'] ?? count($contatos) ?></strong> contatos
            </div>
    ...
    <?php else: ?>
        <div class="empty-state">
            ...
            <?php if (!empty($_GET['busca']) || !empty($_GET['status']) || !empty($_GET['periodo'])): ?>
                <a href="/backend/contato" class="btn-action-primary">
...
<!-- CSS e JS (mesmos do agendamento) -->
<style>
    :root {
        --cor-primaria: #5f7396;
        --cor-acento: #1487df;
    ...
    /* CORES DOS ÍCONES - RESPONDIDOS */
    .stat-respondido .stat-icon {
...
function bulkDelete() {
    ...
    fetch('/backend/contato/deletar-multiplos', {
        method: 'POST',
        body: formData
    })
...
function confirmarExclusao(id) {
    if (confirm("Tem certeza que deseja excluir este contato?")) {
        window.location.href = `/backend/contato/deletar/${id}`;
    }
}

function irParaExclusao(id) {
    if (confirm("Tem certeza que deseja excluir este contato?")) {
        window.location.href = `/backend/contato/excluir/${id}`;
    }
}
