...
class ServicoSiteValidador
{
     ...
     * @param bool $isUpdate Se é uma atualização (foto opcional)
    ...
    public static function validar(array $post, array $files, bool $isUpdate = false): array
    {
        ...
        if (!$isUpdate) {
            ...
            if (empty($files['foto_servico']['name'])) {
                $erros[] = "A foto do serviço é obrigatória.";
            } else {
                $errosImagem = self::validarImagem($files['foto_servico'], 'Foto do Serviço');
                $erros = array_merge($erros, $errosImagem);
            }
        } else {
            // Na ATUALIZAÇÃO, validar apenas se houver nova foto
            if (!empty($files['foto_servico']['name'])) {
                $errosImagem = self::validarImagem($files['foto_servico'], 'Foto do Serviço');
                $erros = array_merge($erros, $errosImagem);
            }
        ...
        if ($isUpdate && isset($post['status_servico'])) {
            $statusValidos = ['Ativo', 'Inativo'];
            if (!in_array($post['status_servico'], $statusValidos)) {
